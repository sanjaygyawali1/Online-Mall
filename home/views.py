...
def category_list(request, *args, **kwargs):
    ...
    # name = get_object_or_404(Category, title=title)
    # name = Category.objects.filter(title = title)[0]
    # name = Category.objects.filter(title = title).first()
    # name = Category.objects.get(title = title)
